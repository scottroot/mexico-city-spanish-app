...
export const getAudio = async (text: string, model?: string, gender?: "man" | "woman") => {
  ...
  if (stream) {
    return stream; // Return the stream directly instead of waiting for buffer
  } else {
...
export async function textToSpeech(options: DeepgramTTSOptions): Promise<DeepgramTTSResponse> {
  const {
    text,
    model = process.env.DEEPGRAM_VOICE_WOMAN,
    // encoding = 'mp3',
    // container = 'mp3',
    // sample_rate = 24000
  } = options;
